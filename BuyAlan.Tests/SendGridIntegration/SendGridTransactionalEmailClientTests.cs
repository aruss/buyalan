...
public class SendGridTransactionalEmailServiceTests
{
    ...
    public async Task SendTemplateAsync_SendsExpectedPayload()
    {
        ...
        await client.SendTemplateAsync(
            ...
            {
                ["confirmation_url"] = "https://buyalan.app/confirm?token=abc"
            });
        ...
        Assert.Equal("notifications@buyalan.app", root.GetProperty("from").GetProperty("email").GetString());
        ...
        Assert.Equal(
            "https://buyalan.app/confirm?token=abc",
