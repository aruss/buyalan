...
public static class AgentEndpoints
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        BuyAlan.Agents.GetSubscriptionAgentsResult result = await service.GetAgentsAsync(
            new GetSubscriptionAgentsInput(input.Subscription, userId.Value),
            cancellationToken);

        if (result is BuyAlan.Agents.GetSubscriptionAgentsResult.Failure failure)
        {
            return MapError(failure.ErrorCode);
        }

        BuyAlan.Agents.GetSubscriptionAgentsResult.Success success =
            (BuyAlan.Agents.GetSubscriptionAgentsResult.Success)result;
        ...
        CancellationToken cancellationToken)
    {
        ...
        BuyAlan.Agents.GetAgentResult result = await service.GetAgentAsync(
            new GetAgentInput(agentId, userId.Value),
            cancellationToken);

        if (result is BuyAlan.Agents.GetAgentResult.Failure failure)
        {
            return MapError(failure.ErrorCode);
        }

        BuyAlan.Agents.GetAgentResult.Success success = (BuyAlan.Agents.GetAgentResult.Success)result;
        return TypedResults.Ok(ToAgentResult(success.Agent));
    }
        ...
        CancellationToken cancellationToken)
    {
        ...
        BuyAlan.Agents.DeleteAgentResult result = await service.DeleteAgentAsync(
            new DeleteAgentInput(agentId, userId.Value),
            cancellationToken);

        if (result is BuyAlan.Agents.DeleteAgentResult.Failure failure)
        {
            return MapError(failure.ErrorCode);
        }
        ...
        CancellationToken cancellationToken)
    {
        ...
        BuyAlan.Agents.GetAgentCatalogProductAccessStateOperationResult accessStateResult = await accessService.GetStateAsync(
            new BuyAlan.Agents.GetAgentCatalogProductAccessStateInput(agentId),
            cancellationToken);

        if (accessStateResult is BuyAlan.Agents.GetAgentCatalogProductAccessStateOperationResult.Failure accessFailure)
        {
            return MapError(accessFailure.ErrorCode);
        }

        BuyAlan.Agents.GetAgentCatalogProductAccessStateOperationResult.Success accessSuccess =
            (BuyAlan.Agents.GetAgentCatalogProductAccessStateOperationResult.Success)accessStateResult;
        ...
        CancellationToken cancellationToken)
    {
        ...
        BuyAlan.Agents.GetAgentSalesZipCodeStateOperationResult result = await zipCodeService.GetStateAsync(
            new BuyAlan.Agents.GetAgentSalesZipCodeStateInput(agentId),
            cancellationToken);

        if (result is BuyAlan.Agents.GetAgentSalesZipCodeStateOperationResult.Failure failure)
        {
            return MapError(failure.ErrorCode);
        }

        BuyAlan.Agents.GetAgentSalesZipCodeStateOperationResult.Success success =
            (BuyAlan.Agents.GetAgentSalesZipCodeStateOperationResult.Success)result;
    ...
    private static AgentCatalogProductAccessStateResult ToAgentCatalogProductAccessStateResult(
        BuyAlan.Agents.AgentCatalogProductAccessStateResult state)
    {
    ...
    private static AgentSalesZipCodeStateResult ToAgentSalesZipCodeStateResult(
        BuyAlan.Agents.AgentSalesZipCodeStateResult state)
    {
