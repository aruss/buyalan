...
public static class WolverineBuilderExtensions
{
    ...
    public static WebApplicationBuilder AddWolverineServices(this WebApplicationBuilder builder)
    {
        builder.Host.UseWolverine(options =>
        {
            ...
            string? databaseConnectionString = builder.Configuration.GetConnectionString("buyalan");
            if (String.IsNullOrWhiteSpace(databaseConnectionString))
            {
                throw new InvalidOperationException("Database connection string 'buyalan' is missing.");
            }
