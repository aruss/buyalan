...
public class SendGridNewsletterClientTests
{
    ...
    public async Task UpsertNewsletterContactAsync_SendsExpectedPayload()
    {
        ...
        SendGridOptions options = new()
        {
            ...
            NewsletterConfirmTemplateId = "d-template",
            NewsletterFromEmail = "hello@example.com"
        };
    ...
    [Fact]
    public async Task SendNewsletterConfirmationEmailAsync_SendsExpectedPayload()
    {
        RecordingHandler handler = new((HttpRequestMessage _) =>
        {
            return new HttpResponseMessage(HttpStatusCode.Accepted);
        ...
        SendGridOptions options = new()
        {
            ApiKey = "sendgrid-api-key",
            NewsletterListId = "newsletter-list-id",
            NewsletterConfirmTemplateId = "d-template-123",
            NewsletterFromEmail = "newsletter@heyalan.app"
        };

        SendGridClient client = new(httpClientFactory, options);
        await client.SendNewsletterConfirmationEmailAsync("person@example.com", "https://heyalan.app/newsletter/confirm?token=abc");
        ...
        JsonDocument payload = JsonDocument.Parse(handler.LastRequestContent!);
        JsonElement root = payload.RootElement;
        Assert.Equal("d-template-123", root.GetProperty("template_id").GetString());
        Assert.Equal("newsletter@heyalan.app", root.GetProperty("from").GetProperty("email").GetString());
        ...
        Assert.Equal(
            "https://heyalan.app/newsletter/confirm?token=abc",
