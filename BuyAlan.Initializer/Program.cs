...
public class Program
{
    ...
    private static async Task<int> Main(string[] args)
    {
        ...
        var connectionString = builder.Configuration.GetConnectionString("buyalan");
        ...
        CancellationToken cancellationToken)
    {
        await pipeline.ExecuteAsync(async token =>
        {
            ...
            //string adminEmail = configuration["ADMIN_EMAIL"] ?? "admin@buyalan.ai";
            //string adminPassword = configuration["ADMIN_PASSWORD"] ?? "admin@buyalan.ai";
        ...
        IHost host, CancellationToken cancellationToken)
    {
        try
        {
            ...
            var vhostName = configuration["RABBITMQ_VHOST"] ?? "buyalan"; // Fallback just in case
