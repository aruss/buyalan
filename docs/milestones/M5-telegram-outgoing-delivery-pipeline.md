...
- [x] Update `BuyAlan.WebApi/TelegramIntegration/TelegramWebhookEndpoints.cs` to inject `MainDataContext`.
...
- [x] Register `OutgoingTelegramMessageConsumer` in `BuyAlan.WebApi/Infrastructure/MassTransitBuilderExtensions.cs`.
- [x] Register `OutgoingTelegramMessageConsumer` in `BuyAlan.Initializer/Program.cs` topology deployment section.
