...
## Goal
Add custom 3rd-party identity endpoints for Google login in `BuyAlan.WebApi` without using `MapIdentityApi<T>`, while preserving the project's cookie-based authentication model behind the frontend proxy.
...
- [x] Extend `BuyAlan/Configuration/AppOptions.cs` with nullable Google credential properties.
...
- [x] Update `BuyAlan/Identity/IdentityBuilderExtensions.cs` to conditionally register Google auth only when both credentials exist.
...
- [x] Update `BuyAlan.WebApi/Identity/IdentityEndpoints.cs` to add:
...
- [x] Add operation-centric DTOs in `BuyAlan.WebApi/Identity`:
  - [x] `GetExternalLoginProvidersResult`
  - [x] `ExternalLoginProviderItem`
...
## Gate I: Google UserInfo Verification Claim Normalization
  - [x] Update Google auth registration in `BuyAlan/Identity/IdentityBuilderExtensions.cs` to explicitly source verification data from
  Google UserInfo endpoint.
    - [x] Configure/confirm `GoogleOptions.UserInformationEndpoint = "https://www.googleapis.com/oauth2/v2/userinfo"`.
    - [x] Update `WithError(...)` logic in `BuyAlan.WebApi/Identity/IdentityEndpoints.cs` to replace existing `authError` rather than
  append duplicates.
