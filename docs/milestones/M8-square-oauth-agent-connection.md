...
### New module
- [x] Create `BuyAlan/SquareIntegration` folder.
- [x] Add `ISquareTokenService` + implementation in `BuyAlan` project.
...
- [ ] Stop and hand off for migration creation/run from `BuyAlan.Initializer` per repository rule.

## Gate C: Shared Square Token Lifecycle Service
- [x] Add `BuyAlan/SquareIntegration/ISquareTokenService.cs`.
...
- [x] `Square` NuGet package is installed in `BuyAlan` (`Square` `43.0.0`).
...
- [ ] Implement `/onboarding` UI flow in `BuyAlan.WebApp` using backend onboarding state as source of truth.
...
- [x] `BuyAlan.WebApp/src/lib/api` already contains generated onboarding endpoints; Gate I MUST use generated API client methods.
- [x] `BuyAlan.WebApp/src/lib/onboarding-api.ts` custom wrapper is not required for Gate I and should be removed in corrective pass.
...
- [ ] Corrective pass required: rebase `BuyAlan.WebApp/src/app/onboarding/page.tsx` on `docs/onboaring_page.tsx` and wire backend behavior without visual changes.
...
- [ ] After Gate B schema changes are implemented, stop and hand off for migration creation/run from `BuyAlan.Initializer` per repository rule:
  - `dotnet ef migrations add Init --context MainDataContext -o .\\Migrations`
- [ ] Rework existing `BuyAlan.WebApp/src/app/onboarding/page.tsx` from local-only wizard state to backend-driven onboarding state + mutation flow while preserving the approved UI exactly (`docs/onboaring_page.tsx` as visual source of truth).
