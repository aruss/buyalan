...
## Goal
Integrate ASP.NET Core Identity into the existing `BuyAlan` solution so parents/admins can sign in via `BuyAlan.WebApp` (Next.js App Router) while `BuyAlan.WebApi` owns user state. Use cookie-based sessions through the existing `/api` proxy and support local email/password plus Google OAuth.

## Scope
- **Backend (`BuyAlan.WebApi`):** Integrate Identity with `MainDataContext` (PostgreSQL via Npgsql) and expose minimal API auth endpoints at `/auth`.
- **Frontend (`BuyAlan.WebApp`):** Implement login/registration UI and route protection using the existing `/api` rewrite to the WebApi.
...
- [x] Create `ApplicationUser` (inherits from `IdentityUser`) in `BuyAlan.Data`.
- [x] Update `MainDataContext` to inherit from `IdentityDbContext<ApplicationUser>` and preserve existing entity mappings.
- [x] Add EF Core migration from `BuyAlan.Initializer` for the Identity schema (PostgreSQL).
- [x] Configure Identity services in `BuyAlan.WebApi/Program.cs` and map Identity API endpoints via `app.MapGroup("/auth").MapIdentityApi<ApplicationUser>()`.
- [x] Seed an admin user via `BuyAlan.Initializer` after migrations; only create when the user does not exist (use `ADMIN_EMAIL`/`ADMIN_PASSWORD`).
- [x] Pass `ADMIN_EMAIL` and `ADMIN_PASSWORD` from `BuyAlan.AppHost` to the initializer; fail fast if missing.
...
- [x] **Frontend:** Use `/api/auth/*` calls (rewritten by `BuyAlan.WebApp/src/proxy.ts`) for login/register/logout.
...
- [x] Add IEmailSender to the BuyAlan.WebApi project, for development cases just send the email to log info stream. 

## Gate D: Frontend Logout Action
- [x] Add client-side logout helper in `BuyAlan.WebApp/src/lib`.
...
- [ ] Normalize `picture` claims and store it in BuyAlan user 
