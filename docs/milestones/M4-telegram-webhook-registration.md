...
## Summary
Add a dedicated Telegram application service in `BuyAlan.WebApi` that can register a webhook per bot token using `Telegram.Bot`.
...
- [ ] Add `ITelegramService` in `BuyAlan.WebApi/Telegram`.
...
- [ ] Keep `AppOptions` sourced from DI singleton registered in `BuyAlan.WebApi/Core/CoreBuilderExtensions.cs`.
...
- [ ] Ensure `builder.AddTelegram()` is called in `BuyAlan.WebApi/Program.cs`.
