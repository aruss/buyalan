...
## Summary
Add a dedicated Telegram application service in `SquareBuddy.WebApi` that can register a webhook per bot token using `Telegram.Bot`.
...
- [ ] Add `ITelegramService` in `SquareBuddy.WebApi/Telegram`.
...
- [ ] Keep `AppOptions` sourced from DI singleton registered in `SquareBuddy.WebApi/Core/CoreBuilderExtensions.cs`.
...
- [ ] Ensure `builder.AddTelegram()` is called in `SquareBuddy.WebApi/Program.cs`.
